...
class Statement:
    ...
    def __init__(self, line, enum_type):
        ...
        if enum_type == StatementType.IF:
            self.expressions.append(Expression(ExpressionType.BOOLEAN, line[1:4]))
        if enum_type == StatementType.PRINT:
            self.expressions.append(Expression(ExpressionType.ARITHMETIC, line[2:5]))
    ...
    def print_statement(self):
        ...
        if self.enum_type == StatementType.IF:
            print("<if_statement> -> if <boolean_expression> <block> else <block> end")
            self.expressions[0].print_expression()

        if self.enum_type == StatementType.PRINT:
